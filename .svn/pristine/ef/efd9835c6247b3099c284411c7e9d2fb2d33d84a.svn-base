    ...
    <div>
        <el-card>
            ...
            <el-table :data="tableData" style="width: 100%" v-loading="loading">
                <el-table-column prop="private_id" label="ID" width="55"></el-table-column>
                ...
                <el-table-column label="操作" width="150">
                    <template slot-scope="scope">
                        <el-button
                            ...
                            @click="changeState(scope.row.private_id, scope.row.resource_id, true)">同意</el-button>
                        <el-button
                            size="mini"
                            plain
                            type="danger"
                            v-if="scope.row.status === 0"
                            @click="changeState(scope.row.private_id, scope.row.resource_id, false)">驳回</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <!-- 分页 -->
...
export default {
    ...
    methods: {
        ...
        changeState(private_id, resource_id, isOn) { // 是否同意 isOn
            if (isOn) { // 同意
                this.$https.post('/api/rent_base_resources/UpdatePrivateStatus',{
        ...
        getDataList() { // 分页获取
            ...
            this.$https.get('/api/rent_base_resources/GetPrivateApply', {
                params: {
                    page: this.pageIndex,
