...
export default {
    ...
    created() {
        this.$https.get('/api/system_config/GetConfig?config_code=rent_inactive').then((result) => {
            if (result.data.code == 0) {
                this.form.config_code = result.data.data.config_code;
                this.form.config_value = result.data.data.config_value;
                this.form.config_unit = result.data.data.config_unit.toString();
            } else {
                this.$message({
                    type: 'error',
                    showClose: true,
    ...
    methods: {
                    ...
                    this.$https.post('/api/system_config/update_system_config_config_value', _postData).then((result) => {
                        if (result.data.code == 0) {
                            this.$message({
                                ...
                                onClose: () => {
                                    // 关闭当前dialog
                                    this.closeDialog('dialogColdConfig');
                                    // 刷新列表
        ...
        closeDialog(name) { // 关闭当前
            ...
            if (!name) {
                this.$emit('closeDialog', 'dialogColdConfig'); // 执行父组件关闭方法
            } else {
